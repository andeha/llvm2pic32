...
#define elf32_filenameOrNULL *ca
    auto open_bootloader_file = ^(const char *utf8path𝘖𝘳𝙽𝚄𝙻𝙻) {
      /* Expand ~ in e․𝘨 `~/myshoebox/mybootloader.hex` */
...
/* 2 */ Elf32_Shdr SectionNames;
    ...
    /* 0xBE00 0000 @ AsPhysical: TB1PGMCFG3 - TB1PGMCFG0 Four 32-bit words: 0xffffffff, 
       0xffffffff, 0xffffffff, 0xF9ffffff */
    /* int nvmcon_invpfswap=-1;
    case 'e': nvmcon_invpfswap=0; return; // Even NVMCON.PFSWAP cycle (2, 4, 6)
    case 'o': nvmcon_invpfswap=1; return; // Odd NVMCON.PFSWAP cycle (1, 3, 5) (default)
    if (nvmcon_invpfswap != -1) {
      if (nvmcon_invpfswap) fprintf(stdout, ":020000041E00C1\x0d\x0a"
         ":10000000fffffffffffffffffffffffff9ffffff06\x0d\x0a");
      else fprintf(stdout, ":020000041E00C1\x0d\x0a"
         ":10000000ffffffffffffffffffffffffffffffff00\x0d\x0a");
     } */
...
/* N+1 */   fixElf32(sectionHeader.offset);
	...
	/* Here we also emits a .svg-renderable SF3-palette displaying .bss, .data, .code, rodata and lines of code */
    return 0;
}
